...
```{r "example02", eval = F}
figure( width = 700, height = 400, title = "Diamonds from ggplot2" ) %>>%
  ly_points(
    x = carat
    , y = price
    , data = diamonds
...
`rbokeh` can do so much that we could write an entire book.  I'll start by replicating many of the examples from `ggplot2` and `lattice` books in this [Gist](https://gist.github.com/timelyportfolio/8300c43ac43d772600df).
