...
`parallel-coordinates` handles bigger data well since the lines are drawn on the `canvas`.  The `diamonds` dataset from `ggplot2` will give us big enough data to test.  Set `queue = T` to progressively render the chart on build and interaction.  I'll sample the `diamonds` to reduce the size of the HTML, but feel free to remove the sample and use the entire dataset on you local machine.
